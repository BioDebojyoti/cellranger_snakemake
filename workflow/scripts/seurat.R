...
libraries2use <- c(
    ...
    "Seurat", "optparse", "dplyr", "remotes", "celldex", "SingleR", "writexl"
)
...
library(seuratter)
option_list <- list(
    make_option(c("-f", "--file"), type = "character", default = "", help = "count matrix data h5 file name", dest = "count_matrix_file"),
    make_option(c("-a", "--aggregate-csv"), type = "character", default = "", help = "aggregate csv file", dest = "aggregate_csv_file"),
    make_option(c("-d", "--directory"), type = "character", default = "", help = "count matrix data directory name", dest = "count_matrix_dir"),
    make_option(c("-o", "--out"), type = "character", help = "output directory", dest = "outdir"),
    make_option(c("--min-cells"), type = "integer", default = as.integer(3), help = "minimum cells [default= %default]", dest = "min.cells"),
    make_option(c("--min-features"), type = "integer", default = as.integer(200), help = "minimum features [default= %default]", dest = "min.features"),
    make_option(c("--max-features"), type = "integer", default = as.integer(2500), help = "maximum features [default= %default]", dest = "max.features"),
    make_option(c("--percent-mt"), type = "double", default = as.double(5), help = "threshold percent mitrochondrial [default= %default]", dest = "cutoff.percent.mt"),
    make_option(c("--percent-rb"), type = "double", default = as.double(5), help = "threshold percent ribosomal [default= %default]", dest = "cutoff.percent.rb"),
    make_option(c("--project"), type = "character", default = "singleCell", help = "output file name [default= %default]", dest = "project")
)
...
metadata <- function(data10X, aggregate_csv_file) {
    # Load metadata
    print(aggregate_csv_file)
    samples <- read.csv(aggregate_csv_file, stringsAsFactors = F)
...
seurat_analysis <- function(
    ...
    # Load the dataset
    if (data_dir != "") {
        data10X <- Read10X(data.dir = data_dir) 
    } else {
        data10X <- Read10X_h5(filename = data_file) 
    }
    ...
    plot4 <- FeatureScatter(seurat_obj, feature1 = "percent.mt", feature2 = "percent.rb") 

    pdf(paste0(seurat_out_dir, "/", project_name, "_features.pdf"))
    cowplot::plot_grid(plot1, plot2, plot3, plot4, labels = c("A", "B", "C", "D"), ncol = 2, align = c("h", "v"), label_size = 20) 
    invisible(dev.off())

    # filter out low quality samples
    seurat_obj <- subset(seurat_obj, nFeature_RNA > min_features & nFeature_RNA < max_features & percent.mt < percent_mt) 
    ...
    for (p in group_pair) {
        ...
        curr_de <- FindMarkers(seurat_obj, ident.1 = p[[1]], ident.2 = p[[2]], group.by = "SingleR.labels", logfc.threshold = log(2)) 
        de_dfs[[contrast]] <- de_dfs
    }

    writexl::write_xlsx(
        x = de_dfs,
        path = paste0(seurat_out_dir, "/",project_name,"_annotated_cluster_DE_results.xlsx")
    )



    saveRDS(seurat_obj, file = paste0(seurat_out_dir, "/", project_name, "_seurat_analysis.Rds")) 
}
